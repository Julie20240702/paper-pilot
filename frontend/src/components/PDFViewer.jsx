...
const PDFViewer = forwardRef(function PDFViewer(
  ...
  const handleZoomOut = () => {
    setZoom((prev) => Math.max(MIN_SCALE, Number((prev - SCALE_STEP).toFixed(1))));
  };

  const handleZoomIn = () => {
    setZoom((prev) => Math.min(MAX_SCALE, Number((prev + SCALE_STEP).toFixed(1))));
  };
    ...
    >
      <div className="h-14 px-4 border-b border-[#E8E2DB] bg-white flex items-center justify-between shadow-[0_1px_2px_rgba(0,0,0,0.04)]">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm text-[#8C7B6E] w-14 text-center">{Math.round(zoom * 100)}%</span>
          <button
            type="button"
